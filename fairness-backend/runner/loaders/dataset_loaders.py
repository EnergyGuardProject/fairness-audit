...
class LoadedDataset:
    ...
    Attributes:
        features: Feature DataFrame (excludes target column; sensitive feature
            columns are retained as the model was trained with them in v1).
...
def load_csv_dataset(config: DatasetConfig, sensitive_features: list[str]) -> LoadedDataset:
    ...
    Returns:
        LoadedDataset with features, y_true, sensitive, and df_full populated.
    ...
    logger.info(
        ...
        path, len(df), len(features.columns),
    )
    return LoadedDataset(features=features, y_true=y_true, sensitive=sensitive, df_full=df)
